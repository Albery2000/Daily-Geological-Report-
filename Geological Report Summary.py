...
if uploaded_file:
    if not OPENPYXL_AVAILABLE:
        st.error("❌ openpyxl is required but not installed. Please add 'openpyxl' to your requirements.txt")
    else:
        try:
            # Load the three sheets with explicit engine specification
            daily = pd.read_excel(uploaded_file, sheet_name="Daily Geological Report", header=None, engine='openpyxl')
            litho = pd.read_excel(uploaded_file, sheet_name="Lithological Description", header=None, engine='openpyxl')
            gas   = pd.read_excel(uploaded_file, sheet_name="Lithology %, ROP & Gas Reading", header=None, engine='openpyxl')
